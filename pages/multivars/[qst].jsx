...
export default function Example() {
  ...
  const submitPca=async()=>{
    try {
      ...
      let response = await axios.post("http://localhost:5000/PCA", bodyContent, {
        headers: headersList
      });
  ...
  return (
    <main className=" py-20 px-20 lg:px-[180px]">
      ...
      <Grid numItemsMd={2} className="mt-6 gap-6">
        ...
        <Card>
          {/* Placeholder to set height */}
          <DynamicBiplotComponent  pcaResult={data} loadings={loadings} variableLabels={selectedvars} />
            ...
            {selectedvarsLda.map((variable, index) => (
              <li
                ...
                onClick={(e)=>{
                  ...
                  setSelectedvarsLda(selectedvarsLda.filter(item => {
                    item !== e.target.innerText}))
